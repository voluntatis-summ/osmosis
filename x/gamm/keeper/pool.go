...
// NewBalancerPool is an internal function that creates a new BalancerPool object with the provided
// parameters, initial assets, and future governor.
func (k Keeper) NewBalancerPool(ctx sdk.Context, poolParams types.PoolParams, assets []types.PoolAsset, futureGovernor string) (types.PoolI, error) {
	poolId := k.getNextPoolNumber(ctx)

	pool, err := types.NewBalancerPool(poolId, poolParams, assets, futureGovernor, ctx.BlockTime())
	if err != nil {
		return nil, err
	}
